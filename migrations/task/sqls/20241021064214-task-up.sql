...
WHERE s.name = '重訓' AND c.user_id IN (
    (SELECT id FROM "USER" WHERE email = 'lee2000@hexschooltest.io'),
    (SELECT id FROM "USER" WHERE email = 'muscle@hexschooltest.io'),
    (SELECT id FROM "USER" WHERE email = 'starplatinum@hexschooltest.io')
);
...
SELECT c.id, s.id
FROM "COACH" c, "SKILL" s
WHERE s.name = '瑜伽' AND c.user_id = (SELECT id FROM "USER" WHERE email = 'muscle@hexschooltest.io');

-- 為教練 Q太郎 新增 有氧運動 專長
INSERT INTO "COACH_LINK_SKILL" (coach_id, skill_id)
SELECT c.id, s.id
FROM "COACH" c, "SKILL" s
WHERE s.name = '有氧運動' AND c.user_id = (SELECT id FROM "USER" WHERE email = 'starplatinum@hexschooltest.io');

-- 為教練 Q太郎 新增 復健訓練 專長
INSERT INTO "COACH_LINK_SKILL" (coach_id, skill_id)
SELECT c.id, s.id
FROM "COACH" c, "SKILL" s
WHERE s.name = '復健訓練' AND c.user_id = (SELECT id FROM "USER" WHERE email = 'starplatinum@hexschooltest.io');
...
-- 新增專長 空中瑜伽 至 SKILL 資料表
INSERT INTO "SKILL" (name)
VALUES ('空中瑜伽');

-- 刪除專長 空中瑜伽 從 SKILL 資料表
DELETE FROM "SKILL"
WHERE name = '空中瑜伽';
...
INNER JOIN "CREDIT_PURCHASE" cph ON cp.id = cph.credit_package_id
WHERE cph.purchase_at >= '2024-11-01' AND cph.purchase_at < '2024-12-01'
GROUP BY cp.name;

-- 6-4. 查詢：計算 11 月份總營收（使用 purchase_at 欄位統計）
-- 顯示須包含以下欄位： 總營收
SELECT 
  SUM(cph.price_paid) AS 總營收
FROM "CREDIT_PURCHASE" cph
WHERE cph.purchase_at >= '2024-11-01' AND cph.purchase_at < '2024-12-01';
...
WHERE cb.created_at >= '2024-11-01' AND cb.created_at < '2024-12-01'
